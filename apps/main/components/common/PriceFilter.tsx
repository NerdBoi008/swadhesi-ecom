...
interface PriceFilterProps {
  minPriceBound: number; // The absolute minimum possible price (e.g., 0)
  maxPriceBound: number; // The absolute maximum possible price across all products
  currentMinPrice: number; // The current min price from the parent's state
  currentMaxPrice: number; // The current max price from the parent's state
  onPriceRangeChange: (min: number, max: number) => void; // Callback with selected range
}
...
export const PriceFilter = ({
  minPriceBound,
  maxPriceBound,
  currentMinPrice, // Destructure the new props
  currentMaxPrice, // Destructure the new props
  onPriceRangeChange,
}: PriceFilterProps) => {
  // Use the props as initial values for the internal state
  const [minPrice, setMinPrice] = useState<number>(currentMinPrice);
  const [maxPrice, setMaxPrice] = useState<number>(currentMaxPrice);

  // State specifically for the slider thumbs to handle intermediate values during slide
  // Initialize with the current prices
  const [sliderValues, setSliderValues] = useState<[number, number]>([currentMinPrice, currentMaxPrice]);

  // --- Synchronization Effect: Update internal state when parent's props change ---
  useEffect(() => {
    // This useEffect will run when currentMinPrice, currentMaxPrice,
    // or the overall bounds change.
    // This ensures the filter visually resets or updates if the parent changes its mind.
    setMinPrice(currentMinPrice);
    setMaxPrice(currentMaxPrice);
    setSliderValues([currentMinPrice, currentMaxPrice]);
  }, [currentMinPrice, currentMaxPrice, minPriceBound, maxPriceBound]); // Include bounds as dependencies

  // --- Effect to report changes back to the parent ---
  // This will run when minPrice or maxPrice (internal states) change
  useEffect(() => {
    // Only report if the internal state is different from what the parent already knows,
    // or if the component just mounted with initial values.
    // Adding a debounce here is often a good idea for sliders to avoid too many updates.
  ...
  }, [minPrice, maxPrice, onPriceRangeChange, currentMinPrice, currentMaxPrice]);
  ...
  const handleSliderValueChange = (values: number[]) => {
    // Update slider visual state immediately
    setSliderValues([values[0] ?? minPriceBound, values[1] ?? maxPriceBound]);
  };

  // Called when slider interaction finishes (on commit)
  const handleSliderCommit = (values: number[]) => {
    setMinPrice(values[0] ?? minPriceBound);
    setMaxPrice(values[1] ?? maxPriceBound);
    // The useEffect above will trigger the onPriceRangeChange callback after a debounce
  };

  const handleMinInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    let value = Number(e.target.value);
    // Clamp value: cannot be negative, greater than current maxPrice, or outside minPriceBound
    value = Math.max(minPriceBound, Math.min(value, maxPrice));
    setMinPrice(value);
    setSliderValues([value, maxPrice]); // Keep slider visual in sync
  };
  ...
  const handleMaxInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    let value = Number(e.target.value);
    // Clamp value: cannot be less than current minPrice, or greater than overall maxPriceBound
    value = Math.min(maxPriceBound, Math.max(value, minPrice));
    setMaxPrice(value);
    setSliderValues([minPrice, value]); // Keep slider visual in sync
  };
