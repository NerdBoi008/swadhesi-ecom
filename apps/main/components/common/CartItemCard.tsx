...
export const CartItemCard = ({
  ...
  const handleNavigation = () => {
    router.push(buildUrl('/products/all/product-details', { productId: product_id }));
  }
    ...
    <Card>
      ...
      <CardContent className='flex gap-5'>
        ...
        <div className='flex flex-col justify-between flex-1 min-w-fit'>
          <div className='space-y-3'>
            {/* Variant Attributes */}
            {Object.entries(variant_attributes).map(([key, value]) => (
              <div key={key} className='flex gap-3 items-center flex-wrap'>
          ...
          <p className='mt-3 font-semibold'>
            ₹ {price_at_purchase.toLocaleString('en-IN', { maximumFractionDigits: 2 })}
      ...
      <CardFooter>
        <div className='flex gap-3 w-full'>
          <Button
            variant={'outline'}
            className='text-red-500 hover:bg-red-50 flex-1 border-red-300 hover:border-red-500'
            onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
              e.stopPropagation();
              removeFromCart(id);
            }}
