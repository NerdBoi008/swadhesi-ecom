...
import { signOut, fetchAuthSession } from 'aws-amplify/auth'; // Keep client-side Amplify imports
import { Customer, Address, AddressType, BaseEntity } from "@repo/types"; // Assuming @repo/types is correct
        ...
        (set, get) => ({
            ...
            checkAuthStatus: async () => {
                ...
                try {
                    ...
                    if (result.success && result.user) {
                        ...
                        console.log("Failed to fetch user profile via server action:", result.message);
                        set({ user: null, addresses: null, isAuthenticated: false, error: result.message || null });
                    }

                } catch (err: any) {
                    console.error("Error in checkAuthStatus (client-side):", err);
                    // Catch errors during fetchAuthSession or server action call
                    set({ user: null, addresses: null, isAuthenticated: false, isLoading: false, error: err.message || "Failed to check auth status." });
                } finally {
                    set({ isLoading: false });
                }
            },
            ...
            fetchUserProfile: async () => {
                ...
                try {
                    ...
                    if (!idToken) {
                        set({ user: null, addresses: null, isAuthenticated: false, isLoading: false, error: "No authenticated user session found." });
            ...
            updateAddress: async (id: string, data: Partial<Address>) => {
                ...
                try {
                    ...
                    const result = await serverUpdateAddress(idToken, id, transformedData); // Pass headers, id, and data
